...
const CHECKED = (state, task) => {
  ...
  if(task.priority === PRIORITY_HIGH){
    state.done.high.count = (task.checked) ? state.done.high.count +1: state.done.high.count -1;
  }else if(task.priority === PRIORITY_MEDIUM){
    state.done.medium.count = (task.checked) ? state.done.medium.count +1: state.done.medium.count -1;
  }else if(task.priority === PRIORITY_LOW){
    state.done.low.count = (task.checked) ? state.done.low.count +1: state.done.low.count -1;
  }
};
...
const isCheckedIcon = (task) => {
  return task.priority + " fa " + ((task.checked) ? "fa-check checked " : "fa-check-square not-checked");
};

const isDevelopped = (task) => {
  return "fa fa-lg fa-2x " + ((task.developped) ? "fa-angle-down" :"fa-angle-left");
};
              ...
              <ul className="fa-ul">
                {
                  task.links.map((link,i) => {
                    return (<li key={link}><a href={link.path} target="_blank"><i className={TypeIcon(link.type)} aria-hidden="true"></i>{link.text}</a></li>)
                  })
                }
              </ul>
